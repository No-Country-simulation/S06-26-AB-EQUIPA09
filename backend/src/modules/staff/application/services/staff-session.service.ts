...
export const createStaffSessionService = (
  ...
  return {
    async createSession(staffId, userAgent, ipAddress) {
      const accessToken  = uniqueToken({ staffId, type: 'staff' },         ACCESS_TTL)
      const refreshToken = uniqueToken({ staffId, type: 'staff_refresh' }, REFRESH_TTL)
    ...
    async refreshSession(refreshToken) {
      ...
      const newAccess  = uniqueToken({ staffId, type: 'staff' },         ACCESS_TTL)
      const newRefresh = uniqueToken({ staffId, type: 'staff_refresh' }, REFRESH_TTL)
